...
1. Azure portal で、Azure AI 検索リソースを参照します。 その後、その **[概要]** ページで、**[データのインポート]** を選択します。
    ...
    ![[データのインポート] が強調表示されている Azure Search Service のスクリーンショット。](./media/overview-panel.png)
    
    > **注:**  Azure AI Search リソースの **[概要]** ページには、ツール バーに次の 2 つのオプションがあります。  
    > - **データのインポート** (クラシック エクスペリエンス)  
    > - **データのインポート (新機能)** (新しいエクスペリエンス)  
    >  
    > さらに、[概要] セクションの下の **[開始する]** パネルには、**新しい UI** にリダイレクトされる **[インポート]** ボタンが含まれています。  
    >  
    > このコースの手順については、**データのインポートのクラシック ワークフロー**を参照してください。 混同を避けるため、必ずツール バーから **[データのインポート]** を選択してください。
...
1. 左側のナビゲーション ウィンドウの **[検索管理]** ビューで **[インデクサー]** ページを表示します。ここには、新しく作成された **margies-indexer** が表示されるはずです。 数分待ち、**[状態]** に "**成功**" と示されるまで **[&#8635; 最新の情報に更新]** をクリックします。
